...
def extract_data(data_type):
    ...
    # read all xml files
    xml_files = glob.glob(os.path.join("data", data_type, "**", "*.xml"), recursive=True)
    for xml_file in tqdm(xml_files, desc="Extracting {} data".format(data_type)):
...
def pad_sentence(sentence, padding):
    ...
    length = len(sentence)
    return sentence[:MAX_LENGTH] if length > MAX_LENGTH else sentence + [padding] * (MAX_LENGTH - length)
...
class AnnotatedDataset(Dataset):
    ...
    def __init__(self, data_type):
        ...
        self.sentences = np.load(path) if os.path.isfile(path) else extract_data(data_type)
        pass
    ...
    def __getitem__(self, idx):
        ...
        attention_mask = torch.tensor([int(label != 0) for label in labels], dtype=torch.long)
